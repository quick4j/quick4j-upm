...
(function (d) {
    function a(g) {
        ...
        if (!h.multiple && !h.editable) {
            d(g).combobox("addEventListener", [
                {name: "onSelect", handler: function (j) {
                    b(g, f, j, h.valueField)
                }},
                {name: "onChange", handler: function (k, j) {
                    if (k == null || k == "") {
    ...
    function e(g) {
        ...
        d(g).combobox("addEventListener", {name: "onLoadSuccess", handler: function () {
            d(g).combobox("textbox").trigger("blur")
        }})
    }
    ...
    function c(k, f, j, h) {
        ...
        switch (f) {
            ...
            case"onUnselect":
                ...
                break;
            default:
                d(k).combo("addEventListener", {name: f, override: h, handler: j});
                break
        }
    }

    d.fn.combobox.defaults.customAttr = {slave: {id: null, remote: true, url: null}};
    d.extend(d.fn.combobox.methods, {followCustomHandle: function (f) {
...
(function (f) {
    function c(j) {
        ...
        if (k.required) {
            ...
            if (k.validType) {
                ...
                f.extend(k, {validType: g})
            }
        }
        f(j).combo("addEventListener", {name: "onChange", handler: function (m, l) {
            if (m == null || m == "") {
    ...
    function e(h) {
        ...
        f(h).combo("textbox").keyup(function (k) {
            if (f(this).val().length != 0 && (f(this).val().length % g.minLength == 0) && g.url) {
                f.ajax({type: "POST", url: g.url, data: {wd: f(this).val()}, dataType: "json", success: function (n) {
    ...
    f.fn.combo.defaults.customAttr = {headervalue: null, autocomplete: {enabled: false, minLength: 3, url: undefined}};
    f.extend(f.fn.combo.methods, {followCustomHandle: function (g) {
        return g.each(function () {
            c(this);
            e(this)
        })
...
(function (a) {
    a.extend(a.fn.combogrid.methods, {getSelected: function (b) {
        return b.combogrid("grid").datagrid("getSelected")
    }})
})(jQuery);
(function (a) {
    a.extend(a.fn.combotree.methods, {followCustomHandle: function (b) {
...
(function (c) {
    function v(A, B, x) {
        ...
        var y = c("#" + z);
        if (y.length == 0) {
            y = c("<div>", {id: z}).menu().menu("appendItems", B)
        }
        return y
    }
    ...
    function n(E) {
        ...
        var y = [
            {text: "显示/隐藏列", iconCls: "icon-columns", submenu: [
                {id: D + "_showAll", text: "全部显示", iconCls: "icon-columns", onclick: function (G, H, F) {
                    c.fn.datagrid.headerContextMenu.defaultEvents.doShowAll(F)
                }},
                {id: D + "_restore", text: "还原", iconCls: "icon-columns", onclick: function (G, H, F) {
        ...
        var A = function (F) {
            return F.substr(F.lastIndexOf("_") + 1, F.length)
        };
        var B = [];
        ...
        function x(G, F) {
            c.each(G, function (I, J) {
                ...
                H._hidden = H.hidden;
                B.push({id: D + "_" + J, text: H.title, disabled: F, iconCls: H.hidden ? "icon-unchecked" : "icon-checked", onclick: function (L, K, O) {
                    var N = A(L.id);
    ...
    function m(A) {
        ...
        c(A).datagrid("addEventListener", {name: "onHeaderContextMenu", handler: function (E, D) {
            E.preventDefault();
            x.menu("addEventListener", [
                {name: "onClick", override: true, handler: function (G) {
                    var F = G.id || G.text;
                    if (z[F]) {
                        z[F].call(this, G, D, A)
                    }
                }},
                {name: "onShow", override: true, handler: function () {
                    C.onShow && C.onShow.call(this, D, A)
                }},
                {name: "onHide", override: true, handler: function () {
    ...
    function j(z) {
        ...
        var x = [
            {id: y + "_delete", text: "删除", iconCls: "icon-remove", onclick: function (B, D, C, A) {
                c.fn.datagrid.rowContextMenu.defaultEvents.doDelete(B, D, C, A)
            }},
            "-",
            {id: y + "_reload", text: "刷新", iconCls: "icon-reload", onclick: function (B, D, C, A) {
                c.fn.datagrid.rowContextMenu.defaultEvents.doReload(B, D, C, A)
            }},
            {id: y + "_reload_this_page", text: "刷新当前页", onclick: function (B, D, C, A) {
    ...
    function e(B) {
        ...
        c(B).datagrid("addEventListener", {name: "onRowContextMenu", handler: function (E, F, D) {
            E.preventDefault();
            c(B).datagrid("selectRow", F);
            x.menu("addEventListener", {name: "onClick", override: true, handler: function (H) {
                var G = H.id || H.text;
                if (A[G]) {
    ...
    function f(D) {
        ...
        var E = {slaveList: F.customAttr.slaveList, activeSlave: F.customAttr.activeSlave};
        var z = c(D);
        var y = [];
        for (var C in E.slaveList) {
            var A = {id: E.slaveList[C].id, params: {}};
            var B = {}, x;
        ...
        if (E.activeSlave == c.fn.datagrid.defaults.customAttr.activeSlave) {
            z.datagrid("addEventListener", {name: "onDblClickRow", handler: function (I, H) {
                for (var G in y) {
                    y[G].params[x] = H[x];
    ...
    function q(C) {
        ...
        var A = function (J) {
            ...
            var E = c("<div>", {id: H}).addClass("dialog-button").appendTo(G).css({position: "absolute", display: "block", "border-bottom": "1px solid #ddd", "border-left": "1px solid #ddd", "border-right": "1px solid #ddd", left: parseInt(F.width() / 2) - x, "z-index": 2013, display: "none", padding: "4px 5px"});
            c('<a href="javascript:void(0)">确定</a>').css("margin-left", "0px").linkbutton({iconCls: "icon-ok"}).click(function () {
            ...
            c('<a href="javascript:void(0)">取消</a>').css("margin-left", "6px").linkbutton({iconCls: "icon-cancel"}).click(function () {
                var K = c(J).datagrid("getRowIndex", c(J).datagrid("getEditingRow"));
        ...
        c(C).datagrid("addEventListener", [
            {name: "onLoadSuccess", handler: function (E) {
                A(this)
            }},
            {name: "onBeforeEdit", handler: function (E, F) {
                A(C);
                B(C, E)
            }},
            {name: "onAfterEdit", handler: function (E, G, F) {
                z(C)
            }},
            {name: "onCancelEdit", handler: function (E, F) {
                z(C)
            }}
        ])
    }
    ...
    function u(z) {
        ...
        var C = function (F, E) {
            var D = {position: y.customAttr.tooltip.position, trackMouse: true, onHide: function () {
                c(F).tooltip("destroy")
            }, onShow: function () {
            ...
            c.extend(D, c.isPlainObject(E) ? E : {content: E});
        ...
        c(z).datagrid("addEventListener", {name: "onLoadSuccess", handler: function (D) {
            A()
        }})
    }

    function b(y) {
        var x = c.extend(true, {}, c.fn.datagrid.defaults, c(y).datagrid("options"));
        if (!x.pagination) {
            return
        }
        c(y).datagrid("addEventListener", {name: "onLoadSuccess", handler: function (z) {
            c(y).datagrid("setPagination", x.customAttr.pagination)
        }})
    }
    ...
    function r(z) {
        ...
        var x = function (C) {
            var B = c(z).datagrid("getPanel");
            if (C.rows.length == 0) {
                var D = c("div.datagrid-view2>div.datagrid-header>div.datagrid-header-inner>table", B)[0];
                var A = c(">div.datagrid-view>div.datagrid-view2>div.datagrid-body", B);
                c("<div>").html("&nbsp;").width(c(D).width()).appendTo(A)
            } else {
                c("div.datagrid-view2>div.datagrid-body>div", B).remove()
            }
        };
        c(z).datagrid("addEventListener", [
            {name: "onLoadSuccess", handler: function (A) {
                x(A)
            }},
            {name: "onLoadError", handler: function () {
                x({rows: []})
            }}
        ])
    }
    ...
    function p(A, z) {
        ...
        var B = c(A).datagrid("getColumnFields", true);
        var y = x.columns[0][0];
    ...
    c.fn.datagrid.headerContextMenu.defaultEvents = {doHideColumn: function (z, y, x) {
        c(z).datagrid("hideColumn", y);
        var A = c(z).datagrid("getHeaderContextMenu");
        A.menu("setIcon", {target: x.target, iconCls: "icon-unchecked"})
    }, doShowColumn: function (z, y, x) {
        c(z).datagrid("showColumn", y);
        var A = c(z).datagrid("getHeaderContextMenu");
        A.menu("setIcon", {target: x.target, iconCls: "icon-checked"})
        ...
        for (i in x) {
            ...
            if (z) {
                B.menu("setIcon", {target: z.target, iconCls: "icon-checked"})
            }
        }
        ...
        for (i in x) {
            ...
            if (!y._hidden) {
                c(A).datagrid("showColumn", x[i]);
                z && B.menu("setIcon", {target: z.target, iconCls: "icon-checked"})
            } else {
                c(A).datagrid("hideColumn", x[i]);
                z && B.menu("setIcon", {target: z.target, iconCls: "icon-unchecked"})
            }
        }
    }};
    c.fn.datagrid.rowContextMenu = {};
    c.fn.datagrid.rowContextMenu.defaultEvents = {doAdd: function (x, A, y, z) {
    ...
    c.extend(c.fn.datagrid.defaults.editors, {my97: {init: function (x, z) {
        var y = c('<input type="text" class="Wdate">').appendTo(x);
        z = z || {};
        z = c.extend({}, z, {readOnly: true});
    ...
    }}, datetimebox: {init: function (x, z) {
        var y = c('<input type="text" class="easyui-datetimebox">').appendTo(x);
        z = z || {};
        z = c.extend({}, z, {formatter: function (A) {
    ...
    }}, numberspinner: {init: function (x, z) {
        var y = c('<input type="text">').appendTo(x);
        z = z || {};
        z = c.extend({}, z, {min: 0, editable: false});
    ...
    }}, combogrid: {init: function (x, z) {
        var y = c('<input type="text">').appendTo(x);
        z = z || {};
        z = c.extend({}, z, {panelWidth: 400, editable: false});
    ...
    c.fn.datagrid.defaults.customAttr = {headerContextMenu: {isShow: false, isMerge: true, items: [], onShow: function (y, x) {
    }, onHide: function () {
    }}, rowContextMenu: {isShow: false, isMerge: true, items: []}, pagination: {showPageList: false, showRefresh: true, beforePageText: undefined, afterPageText: undefined, displayMsg: undefined}, slaveList: undefined, activeSlave: "dblclickrow", rowediting: false, tooltip: {enable: false, target: "row", position: "bottom", fields: undefined, formatter: undefined}, onConfirmEdit: function (x) {
        return true
    }};
    c.extend(c.fn.datagrid.methods, {followCustomHandle: function (x) {
...
(function (a) {
    a.extend({dateFormat: function (c, k) {
        if (c == null) {
            return null
        }
        if (k == null) {
...
(function (b) {
    ...
    b.extend({mask: function (f) {
        f = f || {};
        var d = b.extend({}, {target: "body", loadMsg: b.fn.datagrid.defaults.loadMsg}, f);
        this.unmask(d);
...
(function (c) {
        ...
        if (e && c.isArray(e)) {
            c.each(e, function () {
                ...
                if (c.isPlainObject(j)) {
                    ...
                    if (j == "-") {
                        var g = c(f).menu("appendItem", c.extend({text: j}, h)).menu("findItem", j).target;
    ...
    c.extend(c.fn.menu.methods, {followCustomHandle: function (d) {
...
(function (b) {
    ...
    b.fn.panel.defaults.customAttr = {toolbar: {buttonPosition: "left", data: undefined}};
    b.extend(b.fn.panel.methods, {followCustomHandle: function (d) {
...
(function (a) {
    var b = function (d, h) {
        ...
        a(this).parent().css({left: e, top: n})
    };
    a.fn.dialog.defaults.onMove = b;
    a.fn.window.defaults.onMove = b;
    a.fn.panel.defaults.onMove = b;
    a.fn.panel.defaults = jQuery.extend({}, a.fn.panel.defaults, {onBeforeDestroy: function () {
        var c = a("iframe", this);
        if (c.length > 0) {
...
(function (f) {
    ...
    function n(s) {
        ...
        var r = m(t);
        var q = h(s, t);
        f(s).tabs("addEventListener", {name: "onContextMenu", handler: function (v, w, u) {
            v.preventDefault();
            j(s, q, u);
            q.menu("addEventListener", {name: "onClick", override: true, handler: function (y) {
                var x = y.id || y.text;
                if (r[x]) {
                    r[x].call(this, y, w, u, s)
                }
            }}).menu("show", {left: v.pageX, top: v.pageY})
        }})
    }
    ...
    function j(s, q, o) {
        ...
        if (f.inArray(o, f.fn.tabs.defaults.customAttr.fixedtabs) == -1 && !f(s).tabs("getTab", o).panel("options").closable) {
            q.menu("setText", {target: p, text: f.fn.tabs.defaults.contextMenu.itemname.unfixtab})
        } else {
            q.menu("setText", {target: p, text: f.fn.tabs.defaults.contextMenu.itemname.fixtab});
    ...
    function l(p) {
        var o = g(p);
        return[
            {id: o + "_reload", text: f.fn.tabs.defaults.contextMenu.itemname.reload, onclick: f.fn.tabs.defaults.contextMenu.defaultEventsHandler.reload},
            {id: o + "_fixed", text: f.fn.tabs.defaults.contextMenu.itemname.fixtab, onclick: function (s, t, q, r) {
            ...
            "-",
            {id: o + "_close", text: f.fn.tabs.defaults.contextMenu.itemname.close, onclick: f.fn.tabs.defaults.contextMenu.defaultEventsHandler.closetab},
            {id: o + "_close_others", text: f.fn.tabs.defaults.contextMenu.itemname.closeothers, onclick: f.fn.tabs.defaults.contextMenu.defaultEventsHandler.closeOthersTab},
            {id: o + "_close_rightside", text: f.fn.tabs.defaults.contextMenu.itemname.closerightside, onclick: f.fn.tabs.defaults.contextMenu.defaultEventsHandler.closeRightSideTabs},
            {id: o + "_close_all", text: f.fn.tabs.defaults.contextMenu.itemname.closeall, onclick: f.fn.tabs.defaults.contextMenu.defaultEventsHandler.closeAll}
        ]
    }
    ...
    function d(v, r, o, w, t) {
        ...
        q.panel("body").css({overflow: "hidden"}).empty().append(s);
        if (w) {
    ...
    f.fn.tabs.defaults.contextMenu.defaultEventsHandler = {reload: function (r, t, q, s) {
    ...
    f.fn.tabs.defaults.customAttr = {fixedtabs: [0], contextMenu: {isShow: false, isMerge: true, items: []}};
    var k = f.extend({}, f.fn.tabs.methods);
    f.extend(f.fn.tabs.methods, {followCustomHandle: function (o) {
...
(function ($) {
    function init(target) {
        ...
        var tb = $(target).addClass("datagrid-toolbar").css({"border-top-width": 1});
    ...
    $.fn.toolbar = function (options, param) {
        ...
        return this.each(function () {
            ...
            if (state) {
                ...
                $.data(this, "toolbar", {options: $.extend({}, $.fn.toolbar.defaults, $.parser.parseOptions(this), options)});
                init(this)
            }
        })
    };
    $.fn.toolbar.methods = {options: function (jq) {
    ...
    $.fn.toolbar.defaults = {data: null, url: undefined, buttonPosition: "left", loader: function (success, error) {
        var options = $(this).toolbar("options");
        $.ajax({type: "POST", url: "toolbar_data.json", dataType: "json", success: function (data) {
            success(data)
        }, error: function () {
...
(function (f) {
    ...
    function d(v) {
        var u = k(v);
        return[
            {id: u + "_moveup", text: "位置上移", iconCls: "icon-moveup", onclick: f.fn.tree.contextmenu.defaultEvents.moveup},
            {id: u + "_movedown", text: "位置下移", iconCls: "icon-movedown", onclick: f.fn.tree.contextmenu.defaultEvents.movedown}
        ]
    }
    ...
    function s(y) {
        ...
        var x = q(z);
        var w = r(y, z);
        f(y).tree("addEventListener", {name: "onContextMenu", handler: function (B, A) {
            B.preventDefault();
            f(y).tree("select", A.target);
            w.menu("addEventListener", {name: "onClick", override: true, handler: function (D) {
                var C = D.id || D.text;
                if (x[C]) {
                    x[C].call(this, D, A, y)
                }
            }}).menu("show", {left: B.pageX, top: B.pageY})
        }})
    }
    ...
    function b(v) {
        ...
        if (u.customAttr.expandOnNodeClick) {
            f(v).tree("addEventListener", {name: "onClick", handler: function (w) {
                f(v).tree("toggle", w.target)
            }});
            return
        }
        if (u.customAttr.expandOnDblClick) {
            f(v).tree("addEventListener", {name: "onDblClick", handler: function (w) {
                f(v).tree("toggle", w.target)
            }})
        }
    }
    ...
    function p(v) {
        ...
        f(v).tree("addEventListener", {name: "onBeforeExpand", handler: function (z) {
    ...
    f.fn.tree.contextmenu.defaultEvents = {moveup: function (y, x, z) {
        var v = f.extend(true, {}, f.fn.tree.defaults, f(z).tree("options"));
        var w = l(z, x);
        if (w) {
            var u = f(z).tree("pop", x.target);
            f(z).tree("insert", {before: w.target, data: u});
        ...
        if (v) {
            var u = f(z).tree("pop", x.target);
            f(z).tree("insert", {after: v.target, data: u});
            w.customAttr.onAfterMove.call(this, v, x)
        }
    }};
    f.fn.tree.defaults.customAttr = {idField: null, textField: null, parentField: null, iconField: null, childrenField: null, attributesField: null, attributes: null, dataModel: null, expandOnNodeClick: false, expandOnDblClick: false, onlyNodeExpand: false, contextMenu: {isShow: false, isMerge: true, items: []}, onAfterMove: function (v, u) {
    }};
    ...
    f.extend(f.fn.tree.methods, {followCustomHandle: function (u) {
...
(function (d) {
    ...
    function j(o) {
        ...
        return[
            {id: n + "_delete", text: "删除", iconCls: "icon-remove", onclick: d.fn.treegrid.headerContextMenu.defaultEvents.doRemove},
            "-",
            {id: n + "_reload", text: "刷新", iconCls: "icon-reload", onclick: d.fn.treegrid.headerContextMenu.defaultEvents.doReload}
        ]
    }
    ...
    function m(r) {
        ...
        d(r).treegrid("addEventListener", {name: "onContextMenu", handler: function (u, v) {
            u.preventDefault();
            d(r).treegrid("select", v[n.idField]);
            var t = p.menu("options");
            t.onClickCallback = t.onClickCallback || t.onClick;
            p.menu("addEventListener", {name: "onClick", override: true, handler: function (x) {
                var w = x.id || x.text;
                if (q[w]) {
                    q[w].call(this, x, v, r)
                }
            }}).menu("show", {left: u.pageX, top: u.pageY})
        }})
    }
    ...
    function f(t) {
        ...
        var n = [
            {text: "显示/隐藏列", iconCls: "icon-columns", submenu: [
                {id: s + "_showAll", text: "全部显示", iconCls: "icon-columns", onclick: function (v, w, u) {
                    d.fn.datagrid.headerContextMenu.defaultEvents.doShowAll(u)
                }},
                {id: s + "_restore", text: "还原", iconCls: "icon-columns", onclick: function (v, w, u) {
        ...
        var o = function (u) {
            return u.substr(u.lastIndexOf("_") + 1, u.length)
        };
        var q = [];
        ...
        d.each(r, function (v, x) {
            ...
            u._hidden = u.hidden;
            q.push({id: s + "_" + x, text: u.title, iconCls: u.hidden ? "icon-unchecked" : "icon-checked", disabled: w, onclick: function (z, y, C) {
                var B = o(z.id);
    ...
    function k(q) {
        ...
        d(q).treegrid("addEventListener", {name: "onHeaderContextMenu", handler: function (u, t) {
            u.preventDefault();
            n.menu("addEventListener", {name: "onClick", override: true, handler: function (w) {
                var v = w.id || w.text;
                if (p[v]) {
                    p[v].call(this, w, t, q)
                }
    ...
    function a(q) {
        ...
        if (p.customAttr.expandOnNodeClick) {
            d(q).treegrid("addEventListener", {name: "onClickCell", handler: function (r, s) {
                if (n == r) {
        ...
        if (p.customAttr.expandOnDblClick) {
            d(q).treegrid("addEventListener", {name: "onDblClickCell", handler: function (r, s) {
                if (n == r) {
                    d(q).treegrid("toggle", s[o])
                }
            }})
    ...
    function b(s) {
        ...
        var q = function (y) {
            ...
            var u = d("<div>", {id: x}).addClass("dialog-button").appendTo(w).css({position: "absolute", display: "block", "border-bottom": "1px solid #ddd", "border-left": "1px solid #ddd", "border-right": "1px solid #ddd", left: parseInt(v.width() / 2) - n, "z-index": 2013, display: "none", padding: "4px 5px"});
            d('<a href="javascript:void(0)">确定</a>').css("margin-left", "0px").linkbutton({iconCls: "icon-ok"}).click(function () {
                var z = o.idField;
            ...
            d('<a href="javascript:void(0)">取消</a>').css("margin-left", "6px").linkbutton({iconCls: "icon-cancel"}).click(function () {
                var z = o.idField;
                var A = d(y).treegrid("getEditingRow");
        ...
        d(s).treegrid("addEventListener", [
            {name: "onLoadSuccess", handler: function (v, u) {
                q(this)
            }},
            {name: "onBeforeEdit", handler: function (u) {
                r(s, u)
            }},
            {name: "onAfterEdit", handler: function (v, u) {
                p(s)
            }},
            {name: "onCancelEdit", handler: function (u) {
                p(s)
            }}
        ])
    }
    ...
    function h(p) {
        ...
        var s = function (v, u) {
            var t = {position: o.customAttr.tooltip.position, trackMouse: true, onHide: function () {
                d(v).tooltip("destroy")
            }, onShow: function () {
            ...
            d.extend(t, d.isPlainObject(u) ? u : {content: u});
        ...
        d(p).treegrid("addEventListener", {name: "onLoadSuccess", handler: function (u, t) {
            q()
        }})
    }
    ...
    function c(r, n, q, p) {
        ...
        switch (n) {
            ...
            case"onCancelEdit":
                ...
                break;
            default:
                d(r).datagrid("addEventListener", {name: n, override: p, handler: q});
                break
        }
    }

    d.fn.treegrid.headerContextMenu = {};
    d.fn.treegrid.headerContextMenu.defaultEvents = {doRemove: function (n, p, o) {
    ...
    d.fn.treegrid.defaults.customAttr = {iconField: null, parentField: null, expandOnNodeClick: false, expandOnDblClick: false, headerContextMenu: {isShow: false, isMerge: true, items: []}, contextMenu: {isShow: false, isMerge: true, items: []}, rowediting: false, tooltip: {enable: false, target: "row", position: "bottom", fields: undefined, formatter: undefined}, onConfirmEdit: function (n) {
        return true
    }};
    d.fn.treegrid.defaults.loadFilter = function (p, r) {
        ...
        if (u && u.parentField) {
            ...
            for (var q = 0, s = p.rows.length; q < s; q++) {
                ...
                console.log("async: " + u.async)
                if(u.async){
                    p.rows[q]["state"] = "closed";
                }
            }
        }
        return p
    };
    d.extend(d.fn.treegrid.methods, {followCustomHandle: function (n) {
...
(function (d) {
    ...
    function b(h) {
        var g = d.data(h, "validatebox");
        var f = g.options;
        d(h).tooltip(d.extend({}, f.tipOptions, {content: g.message, position: f.tipPosition, deltaX: f.deltaX})).tooltip("show");
        g.tip = true
    }
    ...
    d.extend(d.fn.validatebox.defaults.rules, {unequal: {validator: function (f, g) {
        return f != g
    }, message: d.fn.validatebox.defaults.missingMessage}, minLength: {validator: function (f, g) {
        return f.length >= g[0]
    }}, equals: {validator: function (f, g) {
        if (/^#/.test(g)) {
            return f == d(g).val()
        } else {
            return f == g
...
(function ($) {
    ...
    $.extend({showWindow: function (options) {
        options = options || {};
        var target;
        var winOpts = $.extend({}, {iconCls: "icon-form", useiframe: false, locate: "top", data: undefined, width: "auto", height: "auto", cache: false, minimizable: true, maximizable: true, collapsible: true, resizable: true, loadMsg: $.fn.datagrid.defaults.loadMsg, showMask: false, onClose: function () {
        ...
        var selfRefrence = {getData: function (name) {
